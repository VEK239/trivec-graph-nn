...
class KnowledgeGraph:
    ...
    def __init__(self, data_path: str = ".",
        ...
        self.ent_maps = None
        self.rel_maps = None
        self._df_train, self._df_val, self._df_test, self._df_test_seen, self._df_test_new, self.df_drug \
    ...
    def load_polypharmacy_data(self) -> Tuple[DataFrame, DataFrame, DataFrame, DataFrame, DataFrame,
                                              np.array]:
        """
        ...
        if self.use_reversed_edges:
            ...
            # get test data only with drugs
            df_test_seen = self.get_df_with_reversed_edges(
                read_csv(self.data_path + DATA_CONST['drug_test_seen']))
            # get test data only with drugs
            df_test_new = self.get_df_with_reversed_edges(
                read_csv(self.data_path + DATA_CONST['drug_test_new']))
        else:
            ...
            df_test_seen = read_csv(self.data_path + DATA_CONST['drug_test_seen'])

            df_test_new = read_csv(self.data_path + DATA_CONST['drug_test_new'])
        ...
        return df_train, df_val, df_test, df_test_seen, df_test_new, df_drug
    ...
    def get_data_by_type(self, data_type: str):
        ...
        if data_type == "test_seen":
            return self._df_test_seen
        if data_type == "test_new":
            return self._df_test_new
        raise ValueError("Unknown datatype!")
